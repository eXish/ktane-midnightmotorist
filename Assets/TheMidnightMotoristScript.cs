/* 
 * 
 *  ----> Remove Alfonso as a human
 *  ---> Change preview pic
 * 
 * Assign music to audio source
 * 
 * Right joystick becomes a volume thing
 * Right blue button toggles music
 * Mute all sounds
 */
...
public class TheMidnightMotoristScript : MonoBehaviour {
   ...
   IEnumerator ShowTestRace () {
      for (int i = 0; i < 3; i++) {
         audio.PlaySoundAtTransform("Dull", transform);
   ...
   IEnumerator ShowFinalRace (bool correct) {
      ...
      if (correct) {
         ...
         audio.PlaySoundAtTransform("Solve SFX", transform);
         ...
         yield return new WaitForSeconds(2.15f);
         GetComponent<KMBombModule>().HandleStrike();
         audio.PlaySoundAtTransform("Strike SFX", transform);
