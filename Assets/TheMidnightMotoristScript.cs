...
public class TheMidnightMotoristScript : MonoBehaviour {
   ...
   List<char> BadCars = new List<char> { };
   ...
   void GenerateRace () {
      ...
      while (usedCars.Count != 4) {
         ...
         while (usedCars.Contains(raceOrder[choice]) || BadCars.Contains(raceOrder[choice]))
      ...
      if (moduleSolved != true) {
         ...
         if (index == 0) {
            ...
            if (!animatingRace) {
               ...
               if (submissionMode) {
                  for (int i = 0; i < BadCarsIndices.Count(); i++) {
                     Selector[BadCarsIndices[i]].gameObject.SetActive(true);
                  }
                  Selector[currentSelection].gameObject.SetActive(true);
                  RoadDelay = .1f;
                  RoadIndex = 0;
                  GoalLine.transform.localPosition = new Vector3(-1.25f, 0.458f, -0.252f);
   ...
   IEnumerator ShowFinalRace (bool correct) {
      ...
      if (correct) {
         ...
         BadCarsIndices.Clear();
         BadCars.Clear();
         for (int i = 0; i < 8; i++) {
            Selector[i].color = new Color32(255, 255, 255, 255);
         }
   ...
   void ResetSelectionTickWhenMoving () {
      if (BadCarsIndices.Contains(currentSelection)) {
         Selector[currentSelection].color = new Color32(255, 0, 0, 255);
      }
      else {
         Selector[currentSelection].gameObject.SetActive(false);
      }
   }

   void SetNewSelectionTick () {
      if (BadCarsIndices.Contains(currentSelection)) {
         Selector[currentSelection].color = new Color32(255, 255, 255, 255);
      }
      else {
         Selector[currentSelection].gameObject.SetActive(true);
      }
   ...
   private void Update () {
      ...
      if (CanMoveLeftStick && !TwitchPlaysActive) {
         ...
         else if (Input.mousePosition.y < MouseLeftPos.y) { //elif necessary so that if player does not move cursor, stick does not go up
            MoveLeftDown = true;
            MoveLeftUp = false;
         }
      }
      ...
      if (CanMoveRightStick && !TwitchPlaysActive) {
         ...
         else if (Input.mousePosition.y < MouseRightPos.y) { //elif necessary so that if player does not move cursor, stick does not go up
            MoveRightDown = true;
            MoveRightUp = false;
         }
      }
